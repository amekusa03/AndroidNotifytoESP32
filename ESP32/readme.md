...
* **解像度**: 320 x 170 ピクセル
* **起動時 Welcome 画面**: ESP32 起動時に中央へ 「Welcome / ESP32 Notify」 画面を10秒間表示（その後自動スリープ移行）。ステータス・初期化メッセージの直書きは排除しクリーンな UI を維持。
* **PC サービス起動通知**: Ubuntu スクリプト (`ubuntu_notifier.py`) の監視デーモン起動時に `🔔 PC Service Start` 通知を自動送信・表示。
